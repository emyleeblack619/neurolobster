...
labels_n = ["AB/PD\n(pacemaker)","LP","PY"]
behaviors = ["forward","forward","forward","turn_left","turn_right","forward","forward","backward"]
behavior_names = {"forward":"→ Forward","backward":"← Backward","turn_left":"↰ Turn Left","turn_right":"↱ Turn Right"}
...
ax_v.set_title("Action Potentials (mV)", color="white", fontsize=9)
ax_v.set_ylim(-85, 65)
ax_v.set_xlim(0, 300)
...
# ── 2. STG Circuit нейронов ───────────────────────────────
ax_stg = fig.add_subplot(gs[0, 2])
ax_stg.set_facecolor("#0a0a18")
ax_stg.set_title("STG Circuit", color="white", fontsize=9)
...
behavior_txt = ax_stg.text(0, 1.75, "→ Forward", ha="center",
...
ax_brain.set_title("Lobster Nervous System — Active Zones", color="white", fontsize=9)
...
ax_heat.set_title("Activity Heatmap", color="white", fontsize=9)
...
ax_heat.set_xticks([])
ax_heat.set_xlabel("← Time", color="#555566", fontsize=7)
...
while plt.get_fignums():
    try:
        ...
        behavior_names_dict = {"forward":"→ Forward","backward":"← Backward",
                                "turn_left":"↰ Turn Left","turn_right":"↱ Turn Right"}
        behavior_txt.set_text(behavior_names_dict.get(behavior, "→ Forward"))
...
with mujoco.viewer.launch_passive(m_mj, d_mj) as viewer:
    ...
    print("Running! Two windows: MuJoCo + Brain Activity\n")
